...
def solve(
    ...
    categories = [GraphCategory.KAMI] if category is None else [category]
    groups = range(1, 19 + 1) if group is None else [group]
    levels = range(1, 6 + 1) if level is None else [level]
    for category in categories:
        for group in groups:
            for level in levels:
                graph_id = make_graph_id(category, group, level)
